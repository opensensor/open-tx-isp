...
static long tx_isp_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    case 0x800856d5: { /* TX_ISP_GET_BUF — OEM EXACT buffer size for MDNS DMA (libimp rmem) */
        ...
        /* Y frame: stride aligned to 8 bytes × height */
        y_frame = ((w + 7) >> 3) * h8;
    ...
    case 0x800856d4: { /* TX_ISP_SET_BUF — OEM EXACT: programs MDNS DMA registers 0x7820-0x786C */
        ...
        if (buf.size >= stats_off) {
            ...
            /* OEM does NOT initialize the MDNS DMA buffer.
             * Previous attempts to zero or fill with 0x80 both caused
             * persistent pink/washed-out UV.  Leave uninitialized to
             * match OEM behavior — MDNS reference converges after 1-2
             * frames of real data. */
